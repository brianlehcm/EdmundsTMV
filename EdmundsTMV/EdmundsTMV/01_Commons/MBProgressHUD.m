...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        _processView = [[UIProgressView alloc] initWithProgressViewStyle:UIProgressViewStyleBar];
        _processView.frame = CGRectMake(50.0, self.center.y + 60.0, [UIScreen mainScreen].bounds.size.width - 100.0, 5.0);
        _processView.progress = 0.5;
        [_processView setTrackTintColor:[UIColor blackColor]];
        [_processView setTintColor:[UIColor whiteColor]];
        _processView.hidden = YES;
        [self addSubview:_processView];
        
        _lblPercent = [[UILabel alloc] initWithFrame:CGRectMake(50.0, self.center.y + 75.0, [UIScreen mainScreen].bounds.size.width - 100.0, 20.0)];
        _lblPercent.textAlignment = NSTextAlignmentCenter;
        _lblPercent.font = [UIFont fontWithName:@"Raleway-SemiBold" size:14.0];
        _lblPercent.textColor = [UIColor whiteColor];
        _lblPercent.text = @"0%";
...
- (void)updateIndicators {
	...
	if (mode == MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
		...
		self.indicator = MB_AUTORELEASE([[PBActivityIndicatorView alloc]
										 initWithActivityIndicatorStyle:kPBActivityIndicatorViewStyleGray]);
...
//	CGSize labelSize = [label.text sizeWithFont:label.font];
    ...
    float heightStr = [detailsLabel.text boundingRectWithSize:maxSize options:NSStringDrawingUsesFontLeading attributes:nil context:nil].size.height;
    
//    float heightStr = [detailsLabel.text heightWithFont:detailsLabel.font contraintSize:maxSize];
    CGSize detailsLabelSize = CGSizeMake(maxSize.width, heightStr);
...
- (id)init {
	return [self initWithFrame:CGRectMake(0.f, 0.f, 60.0f, 60.0f)];
}
